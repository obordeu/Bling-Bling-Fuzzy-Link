...
def checkOmit(matches, var, tol, names):
	matches = matches.map(lambda (k,v): (k,(v,var))).filter(lambda x: filterOmit(x,tol, names)).map(lambda (k,v): (k,v[0]))
	return matches
...
def filterOmit(student, tol, names):
	var = student[1][1]
	name1 = student[1][0][0][1][var]
	name2 = student[1][0][1][1][var]
	otherNames1 = ' '.join([student[1][0][0][1][nms] for nms in names])
	otherNames2 = ' '.join([student[1][0][1][1][nms] for nms in names])
	if Levenshtein.distance(str(name1),str(name2))<=tol:
		return True
	elif name1 == '' or name2 == '':
		return True
